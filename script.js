...
function renderFill(){const c=fillItems[fillIndex];if(!c){$("fillPrompt").textContent=`Finished! ${fillScore} / ${fillItems.length}`;$("fillInput").disabled=true;$("fillCheckBtn").disabled=true;$("fillNextBtn").classList.add("hidden");return}const escaped=en(c).replace(/[.*+?^${}()|[\]\\]/g,"\\$&");$("fillPrompt").textContent=example(c).replace(new RegExp(escaped,"i"),"_____");$("fillProgress").textContent=`Sentence ${fillIndex+1} / ${fillItems.length}`;$("fillScore").textContent=`Score: ${fillScore}`;$("fillInput").value="";$("fillInput").disabled=false;$("fillCheckBtn").disabled=false;$("fillFeedback").textContent="";$("fillNextBtn").classList.add("hidden")}
